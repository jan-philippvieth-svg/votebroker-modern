...
function generatePromoDraft(locale: PromoLocale, analysis: PromoAnalysis): string {
  ...
  const TITLES: Record<string, string> = {
    ...
    ja:  "VoteBroker — Steemでキュレーションシステムを再構築した方法",
...
今日は、私が開発しているオープンソースのプロジェクト **VoteBroker** をご紹介したいと思います。

---

## なぜ作ったのか

Steemでキュレーションをしていると、ある問題に気づきます。良い投稿を支援したいのに、投票パワー（Voting Power）はすぐに消耗してしまう。従来の自動投票ボットは、文脈も、コントロールも、戦略もなく、ただ自動で投票するだけです。

自分にはもっと違うものが必要だと感じました。

---

## VoteBrokerとは何か

VoteBrokerはボットではありません。私の代わりに動くのではなく、**より良い判断を下せるように支援するキュレーションシステム**です。

**開発者について**
私はJan-Philipp Vieth、Steemコミュニティのデベロッパーです。VoteBrokerはオープンソースで、[github.com/jan-philippvieth-svg/votebroker-modern](https://github.com/jan-philippvieth-svg/votebroker-modern) で公開しています。
...
**1. 明示的な同意モデル**
ログイン ≠ 許可。投票の送信、手数料投稿、コミュニティデータ共有など、すべての操作ステップは個別に確認が必要です。各権限はいつでも取り消せます。

**2. USD精度の投票戦略**
パーセンテージウェイトではなく、USD目標値を設定します。VoteBrokerは現在の投票パワーとSTEEM価格をもとに、正確なウェイトを自動計算します。

**3. 投票DNA — 最適なタイミング、最適なウェイト**
過去のキュレーションデータを分析し、キュレーション報酬が最大になるタイミングとウェイトを提案します。

**4. コミュニティインテリジェンス**
オーサーレーダーは、複数の経験豊富なキュレーターが独立して支持している著者を表示します。クオリティコンテンツの強力なシグナルです。

**5. パワースタブルモード**
長期的に投票パワーを安定して維持できる最大ウェイトを自動推薦します。

---

## なぜこれを作ったか

盲目的に投票したくありませんでした。なぜその投票が良いのか、悪いのかを理解したかった。VoteBrokerはブロックチェーンの実データを使って、キュレーションを透明にします。

ツール自体は無料で使えます。小さなサービス手数料は、使用した投票分だけ発生し、指定の手数料投稿への投票として精算されます。トークン移転なし、すべてオンチェーンで完結します。

---

## まだ開発中のこと

VoteBrokerはワークインプログレスです。現在動いているもの：ダッシュボード、投票DNA、コミュニティインテリジェンス、同意システム。今後の予定：

- 管理パネルからの直接投稿機能
- より多くの言語対応とコミュニティ別チューニング
- タイミングインテリジェンスの改善
...
率直なフィードバックをお待ちしています。何が不足しているか、何が役に立つか、良い点・改善点など、何でもお知らせください。`,
  };
